...
const ContactModal = ({ open, onOpenChange }: ContactModalProps) => {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="contact-modal-content !top-1/2 !left-1/2 !z-[1100] max-h-[calc(100svh-32px)] w-[calc(100%-32px)] max-w-[940px] overflow-y-auto rounded-[28px] border border-white/15 bg-black/95 p-0 text-white shadow-2xl shadow-black/50 backdrop-blur-xl sm:rounded-[28px] [&>button]:top-5 [&>button]:right-5 [&>button]:text-white">
        <div className="grid gap-0 lg:grid-cols-[0.86fr_1.14fr]">
          <aside className="border-b border-white/10 bg-white/[0.03] p-6 sm:p-8 lg:border-r lg:border-b-0">
            <p className="mb-4 text-[12px] leading-none font-semibold tracking-[0.18em] text-white/45 uppercase">
              Project inquiry
            </p>
            <DialogTitle className="max-w-[360px] text-[34px] leading-[1.05] font-medium text-white sm:text-[42px]">
              Tell me what you are trying to build.
            </DialogTitle>
            <DialogDescription className="mt-5 max-w-[390px] text-[14px] leading-6 text-white/58">
            ...
            <div className="mt-8 space-y-4 text-[14px] text-white/72">
              ...
              >
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-white/15 bg-white/[0.04]">
